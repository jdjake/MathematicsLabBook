...
The only obstacle to obtaining the full result we desire in the Fourier dimension is understanding a fairly simple situation in probability; Consider a large integer $K$ and a set $W \subset \TT^{dn}$ with $|W| \leq K^{1-n}$. Then take $K$ independant and uniformly distributed random variables $X_1, \dots, X_K$ in $\TT^d$. Let $S$ be the set of indices $k_1 \in \{ 1, \dots, K \}$ such that there exists distinct indices $k_2, \dots, k_n \in \{ 1, \dots, K \}$ such that $(X_{k_1}, \dots, X_{k_n}) \in W$. Standard techniques involving expectation bounds imply that $\#(S) \leq K$ with high probability. For $\xi \in \ZZ^d$, we want to understand what conditions guarantee that with high probability square root cancellation occurs in the sum
%
\[ \frac{1}{K} \sum_{k \in S} e^{2 \pi i \xi \cdot X_k}, \]
%
i.e. under what conditions can we conclude that,
%
\[ \left| \frac{1}{K} \sum_{k \in S} e^{2 \pi i \xi \cdot X_k} \right| \lesssim K^{-1/2} \log(K)^{O(1)}. \]
%
If we write
%
